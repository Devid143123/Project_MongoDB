...
class YOLOPersonCounter:
    def __init__(self, gui_callback, video_source=0, counting_zone_width=30):
        ...
        self.people_records = {}  # Records storing check-in/out info per ID
    ...
    def _detect(self, frame):
        ...
        for result in results.boxes.data.tolist():
            ...
            if int(class_id) == 0 and score > 0.5:
                print(f"[DEBUG] Person detected with confidence {score}")
    ...
    def _check_crossing(self):
        for track_id, track in self.tracks.items():
            ...
            if initial_x < self.line_position and current_x > self.line_position + self.counting_zone_width:
                print(f"[DEBUG] Person {track_id} crossed IN")
                track['crossed'] = True
                self.people_records[track_id] = {
                    "date": date_str,
            ...
            # Crossing right to left (check-out)
                ...
                print(f"[DEBUG] Person {track_id} crossed OUT")
                track['crossed'] = True
                if track_id in self.people_records:
                    self.people_records[track_id]["check_out_time"] = time_str
                    self.people_records[track_id]["direction"] = "OUT"
                else:
                    self.people_records[track_id] = {
                        "date": date_str,
                        "check_in_time": None,
    ...
    def _update_display(self, frame):
        ...
        # Show total counts
        in_count = sum(1 for rec in self.people_records.values() if rec["direction"] == "IN")
        out_count = sum(1 for rec in self.people_records.values() if rec["direction"] == "OUT")

        cv2.putText(frame, f"Checked In: {in_count}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
        cv2.putText(frame, f"Checked Out: {out_count}", (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
...
class PeopleCounterGUI:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("YOLO People Counter Log - MongoDB")
        ...
        self.db_entry.insert(0, "people_counter")
        ...
        self.tree = ttk.Treeview(self.root, columns=("ID", "Date", "Direction", "Check In", "Check Out"), show='headings')
        ...
        self.counter = YOLOPersonCounter(self.add_or_update_entry)
    ...
    def add_or_update_entry(self, pid, direction, time_str):
        existing = None
        ...
        if existing:
            ...
            if direction == "IN":
                self.tree.insert("", "end", values=(pid, date_str, direction, time_str, ""))
            else:
                self.tree.insert("", "end", values=(pid, date_str, direction, "", time_str))

    def refresh_table(self):
        for pid, rec in self.counter.people_records.items():
            existing = None
            for child in self.tree.get_children():
                vals = self.tree.item(child)["values"]
                if vals[0] == pid:
            ...
            if existing:
                vals = list(self.tree.item(existing)["values"])
                vals[1] = rec.get("date", "")
                vals[2] = rec.get("direction", "")
                vals[3] = rec.get("check_in_time", "") or ""
                vals[4] = rec.get("check_out_time", "") or ""
                self.tree.item(existing, values=vals)
            else:
