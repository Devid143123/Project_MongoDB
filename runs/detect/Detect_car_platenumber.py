...
class YOLOVehicleCounter:
    def __init__(self, gui_callback, video_source=None, counting_zone_height=30):
        ...
        YOLO('yolov8n.pt').train(data='Plate_v5.v2i.yolov8/data.yaml', epochs=1, imgsz=640, workers=4)
        # Load license plate detection model (trained for Cambodian plates)
        self.plate_model = YOLO("Plate_v5.v2i.yolov8/data.yaml")  # You need to train or download a Cambodian plate detection model
        
        # Initialize EasyOCR reader for Khmer and Latin characters
        self.reader = easyocr.Reader(['en', 'km'])  # English and Khmer languages
        ...
        # Dictionary to store plate numbers for each vehicle
        self.plate_numbers = {}
    ...
    def _detect_license_plates(self, frame):
        # Detect license plates in the frame
        results = self.plate_model(frame)[0]
        plates = []
        for result in results.boxes.data.tolist():
    ...
    def _read_license_plate(self, plate_img):
        ...
        if results:
            ...
            return plate_text.upper()  # Cambodian plates are usually uppercase
        return None

    def _get_plate_number(self, frame, vehicle_bbox):
        # Get all detected plates in the frame
        plates = self._detect_license_plates(frame)
        
        # Check if any plate is inside the vehicle bounding box
        vx1, vy1, vw, vh = vehicle_bbox
        ...
        for (px1, py1, px2, py2) in plates:
            ...
            if (px1 > vx1 and py1 > vy1 and px2 < vx2 and py2 < vy2):
                ...
                # Read plate number
                plate_number = self._read_license_plate(plate_img)
                if plate_number:
                    return plate_number
        return None
        ...
        for track_id, track in self.tracks.items():
            ...
            if best_iou > 0.3:
                updated_tracks[track_id] = {
                    ...
                    'plate_number': track.get('plate_number', None)  # Keep existing plate number
                }
                assigned_detections.add(best_det_idx)

        for i, det in enumerate(detections):
            if i not in assigned_detections:
                # Try to detect plate number for new vehicles
                plate_number = self._get_plate_number(frame, det['bbox'])
    ...
    def _check_crossing(self):
        for track_id, track in self.tracks.items():
            ...
            if initial_y < self.line_position and current_y > self.line_position + self.counting_zone_height:
                ...
                # Use plate number as ID if available, otherwise use track_id
                vehicle_id = track.get('plate_number', f"ID_{track_id}")
                ...
                self.vehicle_records[track_id] = {
                    ...
                    "plate_number": track.get('plate_number', None)
                }
                self.gui_callback(track_id, "IN", time_str, track['type'], vehicle_id)
                ...
                self.gui_callback(track_id, "OUT", time_str, track['type'], vehicle_id)
    ...
    def _update_display(self, frame):
        ...
        for track_id, track in self.tracks.items():
            ...
            # Display plate number if available, otherwise display track ID
            display_text = track.get('plate_number', f"{track['type']} {track_id}")
...
class VehicleCounterGUI:
    def __init__(self):
        ...
        self.tree = ttk.Treeview(self.root, columns=("ID", "Plate", "Type", "Date", "Direction", "Check In", "Check Out"), show='headings')
    ...
    def add_or_update_entry(self, vid, direction, time_str, vehicle_type, display_id):
        existing = None
        for child in self.tree.get_children():
            vals = self.tree.item(child)["values"]
            if vals[0] == vid:  # Check against original track ID
                existing = child
                break
        ...
        if existing:
            ...
            if direction == "IN":
                vals[1] = display_id if "ID_" not in display_id else ""  # Plate number
                vals[2] = vehicle_type
                vals[3] = date_str
                vals[4] = direction
                vals[5] = time_str
            ...
            plate_number = display_id if "ID_" not in display_id else ""
            if direction == "IN":
                self.tree.insert("", "end", values=(vid, plate_number, vehicle_type, date_str, direction, time_str, ""))
            else:
                self.tree.insert("", "end", values=(vid, plate_number, vehicle_type, date_str, direction, "", time_str))
    ...
    def refresh_table(self):
        for vid, rec in self.counter.vehicle_records.items():
            ...
            if existing:
                vals = list(self.tree.item(existing)["values"])
                vals[1] = rec.get("plate_number", "")  # Plate number
                vals[2] = rec.get("vehicle_type", "")
                vals[3] = rec.get("date", "")
                vals[4] = rec.get("direction", "")
                vals[5] = rec.get("check_in_time", "") or ""
                vals[6] = rec.get("check_out_time", "") or ""
                self.tree.item(existing, values=vals)
            else:
                self.tree.insert("", "end", values=(
                    vid,
                    rec.get("plate_number", ""),  # Plate number
    ...
    def save_to_mongodb(self):
        ...
        try:
            ...
            for vid, rec in self.counter.vehicle_records.items():
                doc = {
                    "vehicle_id": rec.get("vehicle_id", vid),
                    "plate_number": rec.get("plate_number", None),
    ...
    def load_from_mongodb(self):
        ...
        try:
            ...
            for doc in documents:
                ...
                self.tree.insert("", "end", values=(
                    doc.get("_id", ""),  # Using MongoDB _id as the track ID
    ...
    def export_to_excel(self):
        try:
            ...
            df = pd.DataFrame(data, columns=["ID", "Plate", "Type", "Date", "Direction", "Check In", "Check Out"])
